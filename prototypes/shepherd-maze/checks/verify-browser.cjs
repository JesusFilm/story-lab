...
(async()=>{
 ...
 const shot=async name=>{await page.waitForTimeout(1500);await page.screenshot({path:path.join(root,'renders/v4-'+name+'.png')});};
 ...
 try{
  ...
  const started=await state();assert(started.z<133);assert.equal(started.phase,'walking');
  checks.starts_and_walks_without_holding={pass:true};assert.equal(await page.locator('#decision-copy').count(),0);
  ...
  await page.screenshot({path:path.join(root,'renders/v4-running.png')});
  ...
  await page.screenshot({path:path.join(root,'renders/v4-resume-walking.png')});
  ...
  // Inspect actual exported collision bounds; Blender assets should remain untouched.
  ...
  for(const wall of geometry){const index=Number(wall.name.slice(5));assert.deepEqual(wall.rect,map.solid_rectangles_tiles[index]);const[x,z,x2,z2]=wall.rect;
   for(const[a,b]of[[wall.min[0],x/4],[wall.max[0],x2/4],[wall.min[2],z/4],[wall.max[2],z2/4]])assert(Math.abs(a-b)<.001);}
  checks.blender_bounds_still_match={pass:true,walls:geometry.length};
  ...
  await page.screenshot({path:path.join(root,'renders/v4-queued-turn.png')});
  ...
  await mobile.locator('[data-difficulty="maximum"]').tap();await mobile.waitForTimeout(350);await mobile.screenshot({path:path.join(root,'renders/v4-mobile-intro.png')});
  await mobile.locator('#start').tap();await mobile.waitForTimeout(600);
  assert.equal(await mobile.locator('#decision-copy').count(),0);
  ...
  await mobile.screenshot({path:path.join(root,'renders/v4-mobile-deadend.png')});
  ...
  await mobile.screenshot({path:path.join(root,'renders/v4-mobile-play.png')});
  ...
  fs.writeFileSync(path.join(__dirname,'browser-verification.json'),JSON.stringify({date:'2026-09-08',environment:'Local headless Chrome; desktop and emulated touch',checks,errors},null,2)+'\n');
