...
const out = process.env.REVIEW_OUT || "review/11-printed-turn/race-validation";
...
try {
  ...
  results.push({
    check:
      "Interrupted partial stage cannot be stamped with a stale page identity",
    passed: true,
  });
  await page.locator("#next").click();
  await ready();
  ...
  assert.equal(state.scene.printTargetCount, 1);
  ...
  results.push({
    check:
      "Completed stage restores printing and replaces the single live target",
    passed: true,
  });
  ...
  results.push({
    check: "Returning to room releases the print target",
    passed: true,
  });
