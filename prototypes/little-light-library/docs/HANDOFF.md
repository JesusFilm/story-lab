...
**Historical handoff; quality rework remains parked after round 23.** Start with [AGENTS.md](../AGENTS.md), [the current creator guide](creator-guide.md), [architecture](architecture.md) and [the implementation handoff](authoring-handoff.md). The editor is retired; historical setup/acceptance instructions below are not the current workflow. The original acceptance table and [quality log](../review/README.md) describe earlier revisions. Do not infer current performance, visual quality or reference parity from them. Current executed checks and limitations belong in the implementation handoff.
