...
try{
 ...
 while(await blocked.locator('#story-progress').textContent()!=='7 / 7'){await blocked.locator('#story-next').click();await blocked.waitForTimeout(450);}
 await blocked.locator('#story-next').click();assert.match(await blocked.locator('#story-next').textContent(),/Begin adventure/);await blocked.waitForTimeout(5000);assert.equal(await blocked.locator('#story-progress').textContent(),'7 / 7');await blocked.locator('#story-next').click();await blocked.waitForFunction(()=>document.querySelector('#story-overlay').getAttribute('aria-label')==='The shepherds find Jesus');await blocked.close();console.log('PASS: blocked autoplay keeps story playable; final passage waits for Begin.');
