...
try{
 ...
 assert.equal(await page.locator('#loading').isHidden(),true);assert.equal(await page.locator('#story-sound').textContent(),'Sound on');assert.equal(await page.locator('#story-next').textContent(),'Start');
 assert(await page.locator('#story-audio-retry').isHidden());
 await page.locator('#story-next').click();assert.equal(await page.locator('#story-next').textContent(),'Next verse');
 ...
 await page.locator('#story-sound').click();assert.equal(await page.locator('#story-sound').textContent(),'Sound off');
 ...
 await blocked.goto(origin+'/story-preview.html');await blocked.locator('#story-next').click();await blocked.locator('#story-audio-retry').waitFor({state:'visible'});assert(await blocked.locator('#story-overlay').isVisible());await blocked.locator('#story-sound').click();assert.equal(await blocked.locator('#story-sound').textContent(),'Sound off');await blocked.evaluate(()=>window.allowAudio());await blocked.locator('#story-sound').click();
 // Advance to the last passage; it must wait for Begin instead of leaving.
 while(await blocked.locator('#story-next').textContent()!=='Start adventure'){await blocked.locator('#story-next').click();await blocked.waitForTimeout(450);}
 await blocked.waitForTimeout(5000);assert.equal(await blocked.locator('#story-next').textContent(),'Start adventure');await blocked.locator('#story-next').click();await blocked.waitForFunction(()=>document.querySelector('#story-overlay').getAttribute('aria-label')==='The shepherds find Jesus');await blocked.close();console.log('PASS: blocked audio keeps story playable; final passage waits for Start adventure.');
