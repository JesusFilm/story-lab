...
try{
 ...
 await p.locator('#advance').click();await p.waitForFunction(()=>!document.getElementById('sighting-overlay').hidden&&!document.getElementById('sighting-next').disabled);for(let i=0;i<3;i++)await p.locator('#sighting-next').click();await p.waitForFunction(()=>document.getElementById('sighting-overlay').hidden);
