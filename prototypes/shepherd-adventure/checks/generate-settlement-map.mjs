...
const {NODES}=await import('../src/journey-model.mjs');
...
const features=world.settlementFeatures.map(({root,label,kind,asset})=>{
 ...
 root.traverse(o=>{if(o.isMesh){const p=o.geometry.attributes.position;for(let i=0;i<p.count;i++){v.fromBufferAttribute(p,i).applyMatrix4(o.matrixWorld);points.push([round(v.x),round(v.z)]);}}});
...
writeFileSync(new URL('../map/settlement-layout.json',import.meta.url),JSON.stringify(data,null,2)+'\n');
const scale=9,ox=448,oy=878,W=1240,H=1900;
const X=x=>ox+x*scale,Y=z=>oy+z*scale;
const escape=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;');
const colors={'house':'#bdad91','empty-stall':'#ddcfa9','vegetable-stall':'#739b67','pottery-stall':'#779cac','tanner-stall':'#aa7651',pen:'#ac9877',shelter:'#c7a866',workbench:'#d1a35d',prop:'#b4a185',well:'#749bab',gate:'#806344'};
const out=[`<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="100%" height="100%" fill="#f7f4ec"/><style>text{font-family:Arial,sans-serif;fill:#353d36}.label{font-size:12px;font-weight:700;paint-order:stroke;stroke:#f7f4ec;stroke-width:4;stroke-linejoin:round}.small{font-size:13px;fill:#6b7266}.heading{font-size:18px;font-weight:700}</style><text x="64" y="54" font-size="29" font-weight="700">Shepherd Adventure</text><text x="64" y="83" font-size="17">Settlement inventory · top-down map</text><text x="64" y="109" class="small">Actual model footprints and placements · equal scale on X and Z</text>`];
...
for(const p of data.nature.filter(p=>!p.outside||(p.z>45&&Math.abs(p.x)<20)))out.push(`<circle cx="${X(p.x)}" cy="${Y(p.z)}" r="${p.kind==='tree'?13:5}" fill="${p.kind==='tree'?'#b6c3a0':'#c7c6bd'}" opacity=".65"/>`);
...
for(const n of data.nodes.filter(n=>!['wick','oil'].includes(n.id))){out.push(`<circle cx="${X(n.x)}" cy="${Y(n.z)}" r="3.5" fill="#fbf9f1" stroke="#6e715b"/>`);}
const notes=[['INVENTORY',`${features.filter(f=>f.kind==='house').length} houses`,`${features.filter(f=>['vegetable-stall','pottery-stall','tanner-stall'].includes(f.kind)).length} stocked stalls · vegetables, pottery, tanner`,'2 empty stalls','1 animal pen + feeding trough','1 lamp workbench','1 stone well','1 timber gate','1 Nativity shelter'],['READING THE MAP','Solid tan lines: authored routes','Dashed tan: discovery / gate routes','Fine gray dashes: retired supply lanes','Dots: journey stopping points','Green circles: existing trees','Gray walls: perimeter and screens'],['OPEN SPACE','The large northern interior is mostly open.','Only the shelter and its approach occupy it.','House numbers identify instances;','they do not imply assigned inhabitants.'],['SCALE & ORIENTATION','9 pixels = 1 game metre on both axes.','Building outlines include roof overhangs.','The gate is shown closed.','Up is game −Z; right is +X.','This is an inventory plan, not terrain art.']];
let ty=180;
...
out.push(`<path d="M860 1170h90m-90 -6v12m90 -12v12" stroke="#353d36" stroke-width="2"/><text x="905" y="1196" text-anchor="middle" class="small">10 m</text><path d="M1090 1190v-55m-6 9l6 -9l6 9" stroke="#353d36" fill="none" stroke-width="2"/><text x="1090" y="1212" text-anchor="middle" class="small">−Z</text><text x="${X(0)+12}" y="${Y(52)+5}" class="small">Player control begins</text><text x="64" y="1868" class="small">Regenerate with node checks/generate-settlement-map.mjs after changing settlement models or placements.</text></svg>`);
writeFileSync(new URL('../map/settlement-map.svg',import.meta.url),out.join('\n'));
