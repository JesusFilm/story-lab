...
const colors={'house':'#bdad91',annex:'#b5c99b','empty-stall':'#ddcfa9','vegetable-stall':'#739b67','pottery-stall':'#779cac','tanner-stall':'#aa7651',pen:'#ac9877',shelter:'#c7a866',workbench:'#d1a35d',animal:'#ded7c1',prop:'#b4a185',well:'#749bab',gate:'#806344'};
...
for(const wall of data.walls)out.push(`<line x1="${X(wall.a.x)}" y1="${Y(wall.a.z)}" x2="${X(wall.b.x)}" y2="${Y(wall.b.z)}" stroke="${wall.kind==='decoration-wall'?'#5282a3':'#8b897a'}" stroke-width="${wall.width*scale}"/>`);
...
// Facade markers show the actual house-local +X direction in both maps.
for(const f of features.filter(f=>f.kind==='house')){
 const [x,,z]=f.position,dx=Math.cos(f.yaw),dz=-Math.sin(f.yaw);
 const tip={x:x+dx*3.5,z:z+dz*3.5},base={x:x+dx*2.5,z:z+dz*2.5};
 out.push(`<path d="M${X(base.x)} ${Y(base.z)}L${X(tip.x)} ${Y(tip.z)}M${X(tip.x-dx*.65-dz*.4)} ${Y(tip.z-dz*.65+dx*.4)}L${X(tip.x)} ${Y(tip.z)}L${X(tip.x-dx*.65+dz*.4)} ${Y(tip.z-dz*.65-dx*.4)}" stroke="#8644ab" stroke-width="2.5" fill="none"/>`);
}
const labelOffsets={'Feeding trough':[-25,14],'Timber gate':[-14,21],'Lamp workbench':[0,20],'Stone well':[4,-18],'Animal pen':[16,39]};
for(const f of features.filter(f=>!f.label.includes(' decoration ')&&f.kind!=='annex')){const b=f.bounds,cx=(b.min[0]+b.max[0])/2,cz=(b.min[2]+b.max[2])/2;const [dx,dy]=labelOffsets[f.label]||[0,4];out.push(`<text class="label" x="${X(cx)+dx}" y="${Y(cz)+dy}" text-anchor="middle">${escape(f.label)}</text>`);}
...
const notes=[['INVENTORY',`${features.filter(f=>f.kind==='house').length} houses`,`${features.filter(f=>['vegetable-stall','pottery-stall','tanner-stall'].includes(f.kind)).length} stocked stalls · vegetables, pottery, tanner`,'2 empty stalls','2 animal pens + feeding trough','1 lamp workbench','1 stone well','2 timber gates · village and animal area','1 Nativity shelter'],['READING THE MAP','Solid tan lines: authored routes','Dashed tan: discovery / gate routes','Fine gray dashes: retired supply lanes','Dots: journey stopping points','Green circles: existing trees','Gray: enclosure walls; blue: new low walls','Green footprints: six attached annexes','Purple arrows: house door direction','Small brown footprints: 20 decorations'],['OPEN SPACE','The northwest animal area is enclosed.','A curved path reaches the isolated Nativity.','House numbers identify instances;','they do not imply assigned inhabitants.'],['SCALE & ORIENTATION','9 pixels = 1 game metre on both axes.','Building outlines include roof overhangs.','Village gate closed; animal gate open.','Up is game −Z; right is +X.','This is an inventory plan, not terrain art.']];
if(rehearsal)notes.splice(0,notes.length,
 ['REHEARSAL POINTS',...STOPS.map(s=>`${String(s.number).padStart(2,'0')} · ${s.title}`)],
 ['READING THE MAP','Tan line: sampled walking corridor','Numbers: ten placeholder stopping points','Gray: enclosure walls; blue: new low walls','Green footprints: six attached annexes','Purple arrows: house door direction','Small brown footprints: 20 decorations','Tree circles: current rehearsal dressing'],
 ['STRUCTURES','Visited houses face their knocking stops;','retain their accepted centres.','Gate shown closed; opens after point 08.','11 houses, 6 annexes, 20 decorations.'],
