...
const scale=9,ox=448,oy=878,W=1240,H=1430;
...
for(const p of data.nature.filter(p=>!p.outside))out.push(`<circle cx="${X(p.x)}" cy="${Y(p.z)}" r="${p.kind==='tree'?13:5}" fill="${p.kind==='tree'?'#b6c3a0':'#c7c6bd'}" opacity=".65"/>`);
...
out.push(`<path d="M860 1170h90m-90 -6v12m90 -12v12" stroke="#353d36" stroke-width="2"/><text x="905" y="1196" text-anchor="middle" class="small">10 m</text><path d="M1090 1190v-55m-6 9l6 -9l6 9" stroke="#353d36" fill="none" stroke-width="2"/><text x="1090" y="1212" text-anchor="middle" class="small">−Z</text><text x="${X(0)+12}" y="${Y(52)+5}" class="small">Start in field</text><text x="64" y="1398" class="small">Regenerate with node checks/generate-settlement-map.mjs after changing settlement models or placements.</text></svg>`);
