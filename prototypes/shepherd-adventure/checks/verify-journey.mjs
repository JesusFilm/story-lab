...
check(()=>{const j=new Journey();j.start();walk(j,'gate');assert(j.inspected.has('gate'));assert.equal(j.phase,'choice');const before=j.distance;j.commit(j.options.find(e=>e.to==='well'));assert.equal(j.phase,'inspect');assert.equal(j.distance,before);j.closeInspection();walk(j,'hearth');assert.equal(j.phase,'craft');assert(!j.lantern);});
...
function next([at,i,k]){const out=[];if(at==='hearth')out.push([at,i|7,k]);if(at==='lookout')out.push([at,i,k|1]);if(at==='pen')out.push([at,i,k|2]);if(at==='arch')out.push([at,i,k|4]);for(const e of links(at)){if(['wick','oil'].includes(e.to))continue;if(at==='gate'&&!(i&4)&&!['field','hearth'].includes(e.to))continue;if(e.requires==='overlook'&&!(k&1)||e.requires==='rear'&&!(k&2)||['gate','welcome'].includes(e.requires)&&!(k&4))continue;out.push([e.to,i,k]);}return out;}
