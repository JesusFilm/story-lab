...
try{
 ...
 await p.locator('#advance').click();await conversation(p);for(let i=0;i<3;i++)await p.locator('#sighting-next').click();assert.match(await p.locator('.sd-text').textContent(),/closes the door/);await p.locator('#sighting-next').click();assert.equal(await p.locator('#advance').textContent(),'Explore the empty stall');
 ...
 await f.unroute('**/assets/house-9/*.png');await f.locator('#sighting-retry').click();await conversation(f);assert.equal((await state(f)).sighting.failed,false);assert.equal((await state(f)).houseOwner.page,3);await f.locator('#sighting-next').click();await f.close();
