...
const HEIGHTS=[.5,1.2,1.8],OFFSETS=[0,.32,-.32,.65,-.65,1,-1,1.45,-1.45,Math.PI],ELEVATIONS=[3.6,5.4,7.5];
...
// Direct coordinate access avoids the allocation-heavy dynamic-axis hot loop.
// Keep the slab arithmetic identical; camera parity checks cover edge cases.
export function segmentBox(a,b,box,pad=0,endY=b.y){
 let lo=0,hi=1,d,min,max,v1,v2;
 d=b.x-a.x;min=box.min.x-pad;max=box.max.x+pad;
 if(Math.abs(d)<1e-9){if(a.x<min||a.x>max)return null;}
 else{v1=(min-a.x)/d;v2=(max-a.x)/d;lo=Math.max(lo,Math.min(v1,v2));hi=Math.min(hi,Math.max(v1,v2));if(lo>hi)return null;}
 d=endY-a.y;min=box.min.y-pad;max=box.max.y+pad;
 if(Math.abs(d)<1e-9){if(a.y<min||a.y>max)return null;}
 else{v1=(min-a.y)/d;v2=(max-a.y)/d;lo=Math.max(lo,Math.min(v1,v2));hi=Math.min(hi,Math.max(v1,v2));if(lo>hi)return null;}
 d=b.z-a.z;min=box.min.z-pad;max=box.max.z+pad;
 if(Math.abs(d)<1e-9){if(a.z<min||a.z>max)return null;}
 else{v1=(min-a.z)/d;v2=(max-a.z)/d;lo=Math.max(lo,Math.min(v1,v2));hi=Math.min(hi,Math.max(v1,v2));if(lo>hi)return null;}
 return lo;
}
export function blocked(a,b,boxes,pad=.12){for(let i=0;i<boxes.length;i++)if(segmentBox(a,b,boxes[i],pad)!==null)return true;return false;}
