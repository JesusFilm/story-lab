...
const FILES={crickets:'crickets',sheep:'sheep',frog:'frog',jackal:'jackal',voices:'house-murmur'};
...
export function createNightAmbience(context,bus,{load=loadNightRecording,random=Math.random}={}){
 ...
 const counts={sheep:0,jackal:0,frog:0},failures=[];
 let disposed=false,clock=0,nextCall=0,frogWait=8+random()*12;
 const ready=Promise.all(Object.keys(FILES).map(async key=>{
  try{const buffer=await load(context,key);if(!disposed)buffers[key]=buffer;}
 ...
 function loop(id,key,target,dt){
  ...
  if(!item&&target>0&&buffers[key]){
   ...
   gain.gain.value=0;source.connect(gain).connect(bus);source.start(0,key==='voices'?random()*source.buffer.duration:0);
   item={source,gain,level:0};loops.set(id,item);
  }
  if(!item)return;
 ...
 function shot(key,volume){
  ...
  const source=context.createBufferSource(),gain=context.createGain();source.buffer=buffers[key];gain.gain.value=volume;
 ...
 function update(dt,{position,point=-1,lights=[],houses=[],well=null}={}){
  ...
  for(const house of houses){
   ...
   loop(id,'voices',house.open?0:.38*distance**1.6,dt);
  }
  ...
  for(const [key,points,volume] of [['sheep',[0,2,5,6,9],.36],['jackal',[3,8],.16]]){
   const id=`${key}-${point}`;
 ...
 return {ready,update,reset,stop(){disposed=true;reset();},getState:()=>({loaded:Object.keys(buffers),failures:[...failures],counts:{...counts},pending:pending.length,frogWait,levels:Object.fromEntries([...loops].map(([id,item])=>[id,item.level]))})};
}
