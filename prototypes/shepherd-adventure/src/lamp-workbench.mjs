export function createLampWorkbench(journey,onChange){
 ...
 document.body.append(panel);let previousFocus;
 ...
 function render(){
  const visible=journey.phase==='craft';if(visible===panel.hidden){panel.hidden=!visible;document.body.classList.toggle('crafting',visible);if(visible){previousFocus=document.activeElement;q('[data-craft="insert"]').focus();}else previousFocus?.focus?.();}
