...
export class CharacterVariants{
 ...
 playTripo(name,{once=false,duration=null,restart=false}={}){
  ...
  // Keep an already-playing look continuous when inspection interrupts an idle look.
  if(!next||(tripo.current===name&&(!restart||next.isRunning())))return;
  const previous=tripo.current&&tripo.actions[tripo.current];
  // Explicit normalized blending never exposes the bind pose, even when interrupted.
  tripo.blend={elapsed:0,weights:Object.fromEntries(Object.entries(tripo.actions).map(([key,action])=>[key,action.getEffectiveWeight()*(action.isScheduled()?1:0)]))};
  for(const action of Object.values(tripo.actions))action.stopFading();
  ...
  next.setEffectiveWeight(previous?0:1).play();tripo.current=name;
 }
 ...
 updateTripo(dt,controller,movement,paused){
  ...
  }else if(controller.phase==='inspect'&&!paused){
   if(tripo.lastPhase!=='inspect')this.playTripo('lookAround',{once:true,restart:true});
  ...
  tripo.lastPhase=turning?'turning':controller.phase;
  if(tripo.blend){
   tripo.blend.elapsed+=dt;const t=Math.min(1,tripo.blend.elapsed/.16);
   const weights=Object.fromEntries(Object.keys(tripo.actions).map(key=>[key,(tripo.blend.weights[key]||0)*(1-t)+(key===tripo.current?t:0)]));
   const total=Object.values(weights).reduce((a,b)=>a+b,0)||1;
   for(const [key,action] of Object.entries(tripo.actions))action.setEffectiveWeight(weights[key]/total);
   if(t===1){for(const [key,action] of Object.entries(tripo.actions))if(key!==tripo.current)action.stop();tripo.blend=null;}
  }
  tripo.mixer.update(dt);
 }
