...
const story=createJourneyStory({onPlaying(kind){if(kind==='opening')requestAnimationFrame(()=>requestAnimationFrame(prepareGame));},async onClose(kind){
 ...
 try{game.startOpeningCamera();window.storyLoading.ready();}
...
function prepareGame(){
 if(!gamePromise)gamePromise=import('./village-game.mjs').then(module=>module.createVillageGame(story)).catch(error=>{gameError=error;return null;});
 return gamePromise;
}
