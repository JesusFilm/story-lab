...
export function createStoryMedia(){
 ...
 function prepare(kind){
  ...
  const lease={controller:new AbortController(),urls:[],story:null,ready:false};leases.set(kind,lease);
  ...
  lease.promise=(async()=>{
   try{
    ...
    const entries=await Promise.all(urls.map(async url=>{const response=await fetch(url,{signal:lease.controller.signal});if(!response.ok)throw new Error(`Story media HTTP ${response.status}`);const blob=await response.blob();if(lease.controller.signal.aborted)throw new DOMException('Cancelled','AbortError');const local=URL.createObjectURL(blob);lease.urls.push(local);return [url,local];}));
 ...
 return {prepare,release,isReady(kind){return leases.get(kind)?.ready===true;},releaseAll(){for(const kind of [...leases.keys()])release(kind);}};
}
