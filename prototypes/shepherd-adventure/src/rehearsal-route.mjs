...
export class RouteRehearsal{
 ...
 reset(){this.houseRejection=new HouseRejection();this.lampAssembly=new LampAssembly();this.index=-1;this.paused=false;this.travel=null;this.position={...ENTRY,heading:Math.PI};this.phase='choice';this.gait='walk';this.distance=0;this.lantern=false;this.gateOpen=false;this.staged=false;this.completed=new Set();}
 get stop(){return STOPS[this.index]||null;}
 get at(){return this.stop?.id||'entry';}
 get options(){return [];}
 setPriorOutcomes(index){this.houseRejection=new HouseRejection();if(index>1){this.houseRejection.knock();this.houseRejection.tick(7);}this.lampAssembly.reset();if(index>=1)this.lampAssembly.stageComplete();this.lantern=index>=1;this.gateOpen=index>=8;this.completed=new Set(STOPS.slice(0,index).map(s=>s.id));}
 knockOnHouse(){if(this.index!==1||this.travel||this.paused)return false;return this.houseRejection.knock();}
 ...
 step(dt){
  if(!this.paused&&!this.travel&&this.index===1){
   this.houseRejection.tick(dt);const a=this.houseRejection.approach,anchor=STOPS[1].anchor;
   const x=anchor.x+.85*a,z=anchor.z+.85*a;
