...
- Feature checkpoint: `f6c2c6b`, pushed to `codex/shepherd-story-rebuild`.
- Integrated current main while preserving its sermon film comparison and Kokoro
  local-tool changes. Resolved the publication manifest by retaining both sets of
  reviewed runtime entries; regenerated the output inventory.
- Release audit found four missing journey stylesheets and the four lamp assembly
  illustrations. Reviewed and included those explicit runtime dependencies; the
  public build omits the local scene-brief link.
- Portal build and verification passed: 308 files, 409.7 MB, sensitive-content
  checks and dependency checks at `/`, `/story-lab/` and `/story-lab-demos/`.
- Both nativity area and arrival checks passed on the merged tree. Existing Vite
  chunk-size and classic-loader build warnings remain non-blocking. Physical-device
  performance and a fresh full live playthrough are not established by these checks.
- The merge is prepared in an isolated checkout so unrelated local work remains
  untouched. Main push invokes the existing Pages deployment workflow.
