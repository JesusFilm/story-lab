...
for(let i=0;i<3;i++){const u=((time*.16+i*.34)%1),x=-60+u*(w+120),d=(x-center)/85;const jump=Math.abs(d)<1?Math.sin((d+1)*Math.PI/2)*(i===1?89:55):0;ellipse(x,246,23*(1-jump/200),4,'#152e3677');sheep(x,220-jump,.85,i===1&&Math.abs(d)<1?Math.sin((d+1)*Math.PI)*.4:0)}
 ...
 const slow=setInterval(()=>{if(root.hidden)return;const seconds=Math.floor((performance.now()-started)/1000);root.querySelector('.loading-elapsed').textContent=seconds+'s';if(seconds>=20)note.textContent='First visits download the 3D assets. Still working — thanks for waiting.';if(seconds>=60)retry.hidden=false;},1000);
