"""Blender background: prepare a foliage decoration while retaining its source UV atlas.

blender --background --python prepare-decoration.py -- source.glb output-dir --height 1.25 --triangles 150000
"""
import argparse
import json
import math
...
parser.add_argument('--level-normal', type=float, nargs=3, help='Blender-space base normal after --rotate; level it to +Z')
parser.add_argument('--matte', action='store_true', help='Use nonmetallic rough surfaces while retaining base color')
...
if args.level_normal:
    rotation = Vector(args.level_normal).normalized().rotation_difference(Vector((0, 0, 1))).to_matrix().to_4x4() @ rotation
...
camera_data.ortho_scale = max(hi.x-lo.x, hi.y-lo.y, hi.z-lo.z) * factor * 1.3
...
for obj in meshes:
    ...
    # Welding can create duplicate faces in generated foliage; remove those
    # before decimation, otherwise collapsed surfaces become long spikes.
    ...
    # leaves, branches and pots produces black patches on these clusters.
            ...
            if args.matte:
                for shader in material.node_tree.nodes:
                    if shader.type == 'BSDF_PRINCIPLED':
                        for key, value in [('Metallic', 0.0), ('Roughness', 0.95)]:
...
(out / 'mesh-review.json').write_text(json.dumps({
    ...
    'level_normal_after_rotation': args.level_normal, 'matte': args.matte,
