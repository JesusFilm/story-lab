"""Blender background: derive a bounded static prop and render source/runtime views.

blender --background --python prepare.py -- source.glb output-dir --height .28
"""
import argparse
import json
import math
...
for obj in meshes:
    ...
    # Reproject source appearance onto new UVs; reusing collapsed UVs creates
    # conspicuous triangular patches even when the geometry is sound.
    bpy.ops.object.mode_set(mode='EDIT')
    bpy.ops.mesh.select_all(action='SELECT')
    bpy.ops.uv.smart_project(angle_limit=math.radians(66), island_margin=.02)
    bpy.ops.object.mode_set(mode='OBJECT')
    material = bpy.data.materials.new('Baked static prop')
    ...
    scene.render.bake.cage_extrusion = args.height * .025
    scene.render.bake.max_ray_distance = args.height * .05
    scene.render.bake.margin = 8
    scene.render.bake.use_pass_direct = False
    scene.render.bake.use_pass_indirect = False
    scene.render.bake.use_pass_color = True
    for label, bake_type in [('Base color', 'DIFFUSE'), ('Roughness', 'ROUGHNESS'), ('Normal', 'NORMAL')]:
        image = bpy.data.images.new(label, width=args.texture_size, height=args.texture_size, alpha=False)
        if label != 'Base color':
    ...
    links.new(textures['Base color'].outputs['Color'], shader.inputs['Base Color'])
...
(out / 'mesh-review.json').write_text(json.dumps({
    ...
    'note': 'Weld, decimate, unwrap and bake source base color, roughness and tangent normals. Static prop; not rigged.'
}, indent=2) + '\n')
