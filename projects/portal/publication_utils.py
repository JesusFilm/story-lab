...
def prototype_card(proto,retrospective_link=''):
    ...
    return f'''<article class="prototype-card"><a class="prototype-preview" href="{entry}" aria-label="Play {html.escape(proto['title'])}"><img src="{image}" alt="{html.escape(proto.get('image_alt', 'Saved scene from '+proto['title']))}" width="960" height="600"></a><div class="card-copy"><span class="eyebrow">Playable prototype</span><h2>{html.escape(proto['title'])}</h2><p>{html.escape(proto['description'])}</p><div class="card-actions"><a class="action" href="{entry}">Play prototype <span aria-hidden="true">↗</span></a>{retrospective_link}</div></div></article>'''
