...
- `mary-seated-reference.png` and `.prompt.txt`: isolated ImageGen reference derived from the ending artwork.
- `mary-seated-tripo-source.glb`: original generated model.
- `mary-seated-tripo-provenance.json`: generation settings and source/reference hashes.
...
- `mary-seated-tripo-idle.glb` and `.blend`: current runtime model and editable three-bone upper-body rig.
...
The seated Pixal3D source, `prepared/`, `mary-seated-idle.glb`, its editable rig and `renders/` retain the earlier staging model for comparison. The standing reference, Tripo source and automatic rig are separate development variants and are not used in the current scene.
